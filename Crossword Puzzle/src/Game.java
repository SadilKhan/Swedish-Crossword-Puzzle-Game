...
public class Game implements ActionListener, FocusListener,GameInterface{
	...
	public void createStatement() {
		...
		JPanel acrossPanel=GameUtil.createPanel(acrossClues);
		// Down Panel for down statements
		JPanel downPanel=GameUtil.createPanel(downClues);
		...
		if (e.getSource()==checkButton) {
			...
			for(int i=0;i<totalGraySquare;i++) {
				...
				if (text!=null) {
					...
					JOptionPane.showMessageDialog(null, "Sorry!! Wrong Answer","Wrong Answer",JOptionPane.WARNING_MESSAGE,winnerImage);
				}
			...
			if (solution.matches(userSolution)) {
				...
				JOptionPane.showMessageDialog(null, "Yayy!! Correct Answer","Congratulations",JOptionPane.PLAIN_MESSAGE,winnerImage);
			}
			else {
				ImageIcon winnerImage=new ImageIcon(pathString+"wrong_answer.png");
				JOptionPane.showMessageDialog(null, "Sorry!! Wrong Answer","Wrong Answer",JOptionPane.WARNING_MESSAGE,winnerImage);
